...
@members {
    ...
    void writeIntoAssemblyFile(boolean opt, String... lines) {
        for (String line: lines) {
            try {
                ...
                if (opt) {
                    Main.optAssemblyFile.write(line);
                    Main.optAssemblyFile.newLine();
                    Main.optAssemblyFile.flush();
                }
            } catch (IOException e) {
    ...
    void writeCode(String... codeLines) {
        for (String codeLine: codeLines) {
            ...
            switch (tokens[0]) {
                case "MOV":
                    String src = tokens[2];
                    String dest = tokens[1].substring(0, tokens[1].length() -1);
            ...
            prevLine = codeLine;
            writeIntoAssemblyFile(opt, CodeLine);
        }
    }
    ...
    String getOffset(String idName, boolean onlyOffset) {
        ...
        if (onlyOffset)
            return isNumeric ? "[BP + SI]" : offset + "[BX]";
    ...
    void writeLabel(String label) {
        String Label = label + ':';
        writeIntoAssemblyFile(true, Label);
        prevLine = Label;
    }
...
func_definition
    ...
    {
        if (isInGlobal) {
            writeIntoAssemblyFile(true, ".CODE");
            isInGlobal = false;
        }

        String funcName = $ID.getText();
        writeIntoAssemblyFile(true, funcName + " PROC");
    ...
    cs=compound_statement [$pl.start == null ? null : $pl.pms, null]
    {
        if ($cs.retLabel != null)
...
compound_statement [List<String[]> pms, String RetLabel]
    ...
    ss+=statement [
        null,
        $ss.size() == 0 ? $RetLabel :
        $ss.get($ss.size() - 1).retLabel
    ]* RCURL
    {
        $retLabel = $ss.size() == 0 ? $RetLabel :
        $ss.get($ss.size() - 1).retLabel;
...
statement [String nextLabel, String RetLabel]
    returns [String retLabel]
    :
    var_declaration
    ...
    | cs=compound_statement [null, $RetLabel]
    {
        $retLabel = $cs.retLabel;
    }
    ...
    RPAREN 
    s=statement [null, $RetLabel]
    {
        $retLabel = $s.retLabel;
        writeCode("JMP " + nextLbl);
        writeLabel($es2.falseLabel);
    }
    | IF LPAREN e=expression [null, null]
    {
    ...
    RPAREN
    s1=statement [$nextLabel, $RetLabel]
    {
        $retLabel = $s1.retLabel;
    }
    (
        ELSE
        {
        ...
        s2=statement [$nextLabel, $RetLabel]
        {
            $retLabel = $s2.retLabel;
        }
    )?
    {
        if ($s2.start == null) 
    ...
    {
        writeCode(
            mov("AX", getOffset($ID.getText(), false)),
            "CALL print_output",
            "CALL new_line"
        );
    }
    | RETURN expression [null, null] sm=SEMICOLON
    {
        $retLabel = $RetLabel == null ? newLabel() : $RetLabel;  
...
expression_statement 
    ...
    : e=expression [null, null]? SEMICOLON
    {
        if ($e.start != null) {
            $trueLabel = $e.trueLabel;
            $falseLabel = $e.falseLabel;
...
variable returns [String idName, int line]
    : ID (LTHIRD e=expression [null, null] RTHIRD)?
    {
        $idName = $ID.getText();
        $line = $ID.getLine();
        if ($e.start != null)
...
expression [String TrueLabel, String FalseLabel]
    returns [String trueLabel, String falseLabel]
    : (v=variable ASSIGNOP)? le=logic_expression [$TrueLabel, $FalseLabel]
    {
        ...
        if ($v.start != null) {
            ...
            writeCode(mov(getOffset($v.idName, false), "AX"));
        }
    }
    ;

logic_expression [String TrueLabel, String FalseLabel]
    returns [String trueLabel, String falseLabel]
    : (re1=rel_expression [null, null] lo=LOGICOP)?
    {
        ...
        if ($re1.start != null) {
            ...
            if ($re1.trueLabel != null) {
                switch ($lo.getText()) {
                    case "||":
                        $trueLabel = ($TrueLabel == null) ? $re1.trueLabel : $TrueLabel;
                        $falseLabel = nextLabel;
                        writeLabel($re1.falseLabel);
                        break;
                    case "&&":
                        $falseLabel = $re1.falseLabel;
                        $trueLabel = ($TrueLabel == null) ? nextLabel : $TrueLabel;
    ...
    re2=rel_expression [$trueLabel, $falseLabel]
    {
        if ($re1.start != null) {
            if ($re1.trueLabel == null) {
                writeCode(
                ...
                System.out.println($re2.trueLabel);
                System.out.println($re2.falseLabel);
            }
        } else {
            $trueLabel = $re2.trueLabel;
            $falseLabel = $re2.falseLabel;
...
rel_expression [String TrueLabel, String FalseLabel]
    returns [String trueLabel, String falseLabel]
    : (se1=simple_expression [null, null] ro=RELOP)?
    {
        if ($se1.start != null) {
            writeCode(mov("DX", "AX"));
        }
    }
    se2=simple_expression [$TrueLabel, $FalseLabel]
    {
        if ($se1.start != null) {

            String ins = "J";
            $trueLabel = ($TrueLabel == null) ? newLabel() : $TrueLabel;
            $falseLabel = ($FalseLabel == null) ? newLabel() : $FalseLabel;
            ...
            $trueLabel = $se2.exTrueLabel;
            $falseLabel = $se2.exFalseLabel;
            
        }
    }
    ;

simple_expression [String TrueLabel, String FalseLabel]
    returns [String exTrueLabel, String exFalseLabel]
    : t1=term [$TrueLabel, $FalseLabel]
    {
        $exTrueLabel = $t1.exTrueLabel;
        $exFalseLabel = $t1.exFalseLabel;
    }
    (
        ao=ADDOP
        {
...
term [String TrueLabel, String FalseLabel]
    returns [String exTrueLabel, String exFalseLabel]
    : ue1=unary_expression [$TrueLabel, $FalseLabel]
    {
        $exTrueLabel = $ue1.exTrueLabel;
        $exFalseLabel = $ue1.exFalseLabel;
    }
    (
        mo=MULOP
        {
...
unary_expression [String TrueLabel, String FalseLabel]
    returns [String exTrueLabel, String exFalseLabel]
    : pre=(ADDOP | NOT) ue=unary_expression [null, null]
    {
        writeCode(
            "NEG AX"
        );
    }
    | f=factor [$TrueLabel, $FalseLabel]
    {
        $exTrueLabel = $f.exTrueLabel;
        $exFalseLabel = $f.exFalseLabel;
    }
    ;

factor [String TrueLabel, String FalseLabel]
    returns [String exTrueLabel, String exFalseLabel]
    : v=variable do=(INCOP | DECOP)?
    {

        writeCode(
            mov("AX", getOffset($v.idName, false))
        );
        ...
        if ($do != null) {
            writeCode(
                ...
                + " AX",
                mov(getOffset($v.idName, true), "AX"),
                "POP AX"
            ); 
        }
    ...
    | LPAREN e=expression [$TrueLabel, $FalseLabel] RPAREN
    {
        $exTrueLabel = $e.trueLabel;
        $exFalseLabel = $e.falseLabel;
    }
...
arguments
    : logic_expression [null, null]
    {
        writeCode("PUSH AX");
    }
    (
        COMMA logic_expression [null, null]
        {
            writeCode("PUSH AX");
        }
    )*
